-- Drop the ambiguous function that takes UUID
-- We are standardizing on TEXT for user IDs to support both UUIDs and other ID formats
DROP FUNCTION IF EXISTS public.can_make_ai_call(uuid);

-- Ensure the TEXT version exists (re-applying from supabase-fix-all-id-types.sql just in case)
