...
// Simple Error component since @/components/ui/error doesn't exist yet
const Error = ({ children, size }: { children: React.ReactNode; size?: "small" | "large" }) => {
  const textSize = size === "large" ? "text-base" : "text-sm";
  return <div className={`text-red-500 ${textSize}`}>{children}</div>;
};
      ...
      <div className={clsx(
        ...
        <select
          ...
          className={clsx(
            ...
            disabled ? "cursor-not-allowed bg-neutral-900/50 text-neutral-500 border-neutral-800" : variant === "default" ? "text-white bg-[#1A1A1A] border-[#333] cursor-pointer hover:bg-[#222]" : "bg-transparent text-neutral-400",
            error ? "border-red-500 ring-red-900/20 ring-[3px]" : `ring-neutral-700/50 focus:ring-[3px] ${variant === "default" ? "border-[#333]" : "border-transparent ring-none"}`
          )}
        >
