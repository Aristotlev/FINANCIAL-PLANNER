...
export function StockTransactionsView() {
  ...
  useEffect(() => {
    async function loadTransactions() {
      try {
        const data = await SupabaseDataService.getStockTransactions();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 className="text-2xl font-bold text-white">Stock Transactions</h2>
            <p className="text-gray-400 text-sm">History of your stock buy and sell orders</p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-[#141414]">
                ...
                <th className="px-6 py-4 font-medium text-gray-400 text-right">Shares</th>
                ...
                filteredTransactions.map((tx) => (
                  <tr key={tx.id} className="group hover:bg-gray-900/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-gray-300 font-medium whitespace-nowrap">
                        {tx.shares} <span className="text-gray-500 text-xs ml-0.5">{tx.symbol}</span>
                    </td>
                ...
                <tr>
                    <td colSpan={6} className="px-6 py-16 text-center">
                        <div className="flex flex-col items-center justify-center text-gray-500">
                            ...
                            <p className="text-sm">Your stock transaction history will appear here</p>
                        </div>
                    </td>
                </tr>
              )}
