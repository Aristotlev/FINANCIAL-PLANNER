...
export function CryptoAIAnalyticsView() {
  const { cryptoHoldings } = usePortfolioContext();
  ...
  const generateAnalysis = async () => {
    if (cryptoHoldings.length === 0) {
      setAnalysis("You don't have any crypto holdings to analyze yet. Add some positions to get AI insights.");
      return;
    }
    ...
    try {
      ...
      const holdingsSummary = cryptoHoldings.map(h => 
        `- ${h.name} (${h.symbol}): ${h.amount} units, value ${h.value}, entry ${h.entryPoint}`
      ).join('\n');

      const totalValue = cryptoHoldings.reduce((sum, h) => sum + (typeof h.value === 'string' ? parseFloat(h.value) : h.value), 0);

      const prompt = `
        I am a crypto investor with the following portfolio:
        
        ${holdingsSummary}
        
        Total Portfolio Value: $${totalValue}

        Please act as an expert financial crypto analyst. Provide a comprehensive analysis of my portfolio including:
        1. Portfolio Allocation & Diversification Rating (1-10)
        2. Risk Assessment (Low/Medium/High) and why
        3. Strengths of this composition
        4. Weaknesses or potential exposure risks
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">Deep learning analysis of your crypto assets</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#0D0D0D] border border-gray-800 rounded-xl p-6">
          ...
          <p className="text-3xl font-bold text-white">{cryptoHoldings.length}</p>
        ...
        <div className="bg-[#0D0D0D] border border-gray-800 rounded-xl p-6">
          ...
          <p className="text-3xl font-bold text-white">
            {cryptoHoldings.length > 0 ? 
                cryptoHoldings.reduce((prev, current) => (prev.value > current.value) ? prev : current).name 
                : "—"
            }
          </p>
