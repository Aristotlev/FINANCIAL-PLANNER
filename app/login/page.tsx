...
function LoginForm() {
  ...
  const { login, loginWithGoogle } = useBetterAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.toLowerCase().includes('invalid email') || errorMessage.toLowerCase().includes('password') || errorMessage.toLowerCase().includes('credential')) {
        setError(
          <div className="flex flex-col gap-1">
            <span>Invalid email or password.</span>
            <Link href="/forgot-password" className="underline font-bold hover:text-white text-xs mt-1">
