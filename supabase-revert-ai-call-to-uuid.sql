-- Drop both potential signatures to ensure no ambiguity
DROP FUNCTION IF EXISTS public.can_make_ai_call(text);
DROP FUNCTION IF EXISTS public.can_make_ai_call(uuid);

-- Recreate with UUID parameter as that matches the underlying data type
...
COMMENT ON FUNCTION public.can_make_ai_call(UUID) IS 'Check if user can make another AI assistant call';
